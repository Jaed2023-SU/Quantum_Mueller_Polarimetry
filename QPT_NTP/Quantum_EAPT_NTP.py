...
class Quantum_EAPT_NTP:
    ...
    def kraus_to_chi(kraus: list[np.ndarray]) -> np.ndarray:
        ...
        """
        P = Quantum_EAPT_NTP.pauli_matrices()
    ...
    @staticmethod
    def plot_chi(chi: np.ndarray, title_prefix: str = r"$\chi$ $matrix$", save_name: str | None = None) -> None:
        """Plot real and imaginary parts of chi as annotated heatmaps."""
        chi = np.asarray(chi)
        labels = Quantum_EAPT_NTP.PAULI_ORDER
    
        for part_name, data in [("Real", chi.real), ("Imaginary", chi.imag)]:
