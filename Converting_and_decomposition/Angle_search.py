...
def QWP_rot(fast_axis_angle_degree):
    fast_axis_angle = fast_axis_angle_degree * np.pi / 180
    
    Rotation = np.eye(4)
    Rotation[1:3, 1:3] = [[np.cos(2 * fast_axis_angle), np.sin(2 * fast_axis_angle)],
                          [-np.sin(2 * fast_axis_angle), np.cos(2 * fast_axis_angle)]]
...
while res_comp > epsilon:
    iteration += 1
    initial_guess += np.asarray([1.0, 1.0, 1.0])
    Total_result, SS_comp, res_comp = comp_angle(SS, Target = Target, initial_guess = initial_guess)
