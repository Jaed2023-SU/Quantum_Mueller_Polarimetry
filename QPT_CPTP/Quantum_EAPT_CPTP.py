...
class Quantum_EAPT_CPTP:
    ...
    def save_fit_return_values_as_csv_tables(fit: dict, folder: str, prefix: str = "fit"):
        ...
        for key, value in fit.items():
            if key == "chi" or key == "J" or key == "rho_out" or key == "predicted":
                ext_result_folder = folder + "/result"
                os.makedirs(ext_result_folder, exist_ok=True)
                chi_matrix = np.asarray(fit["chi"])
                Quantum_EAPT_CPTP._save_complex_matrix_tables(ext_result_folder, prefix, "chi", chi_matrix)
                J_matrix = np.asarray(fit["J"])
                rho_out_matrix = np.asarray(fit["rho_out"])
                predicted_matrix = np.asarray(fit["predicted"])
                Quantum_EAPT_CPTP._save_complex_matrix_tables(ext_result_folder, prefix, "J", J_matrix)
                Quantum_EAPT_CPTP._save_complex_matrix_tables(ext_result_folder, prefix, "rho_out", rho_out_matrix)
                Quantum_EAPT_CPTP._save_complex_matrix_tables(ext_result_folder, prefix, "predicted", predicted_matrix)
                continue
